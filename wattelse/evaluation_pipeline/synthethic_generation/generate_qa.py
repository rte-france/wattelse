...
def prepare_chunk_batches(
    ...
    for _ in range(n_generations):
        ...
        while len(current_batch) < docs_per_qa * chunks_per_doc and len(used_docs) < len(chunks_by_doc):
            ...
            doc_chunks_remaining = {
                ...
                if sum(1 for used in usage if not used) > 0  # Only consider docs with available chunks
            }
            ...
            available_chunks = [
                (idx, chunk) 
                for idx, (chunk, used) in enumerate(zip(doc_chunks, chunk_usage[selected_doc])) 
                if not used
            ]
...
def generate_qa_pairs(
    ...
    with tqdm_joblib(tqdm(desc="Generating QA pairs", total=actual_generations)):
        outputs = Parallel(n_jobs=-1)(
            delayed(process_single_generation)(batch)
    ...
    for doc_name, chunks in chunks_by_doc.items():
        ...
        total = len(chunks)
        logger.success(f"{Path(doc_name).name}: {used}/{total} chunks used ({(used/total)*100:.2f}% utilization)")
