...
def create_metrics_summary(experiments_data):
        ...
        for metric in sorted(all_metrics):
            ...
            for judge, df in exp["dfs"].items():
                ...
                if score_col in df.columns:
                    ...
                    # For correctness metric, collect all individual scores for CI calculation
                    if metric == "correctness":
                        # Convert to binary scores (0=not good, 1=good) for each judgment
            ...
            if judges_values:
                ...
                # Calculate confidence interval for correctness metric
                ci_lower, ci_upper = (0, 0)
                if metric == "correctness" and all_scores:
                    # Convert percentages to proportions (0-1) for CI calculation
                    ci_lower, ci_upper = calculate_confidence_interval([s * 100 for s in all_scores])
    ...
    for metric, data in metric_summary.items():
        if data:
            ...
            # For correctness, add error bars using CI
            if metric == "correctness":
                error_y = []
        ...
        for metric in sorted(all_metrics):
            ...
            for judge, df in exp["dfs"].items():
                ...
                if score_col in df.columns:
                    ...
                    # For correctness, collect individual scores for CI
                    if metric == "correctness":
                        binary_scores = [1 if score in [4, 5] else 0 for score in df[score_col].dropna()]
                        all_scores.extend(binary_scores)
    ...
    for i, metric in enumerate(sorted(all_metrics)):
        if metric in overall_df.columns:
            ...
            # Add error bars for correctness
            if metric == "correctness":
                error_y = []
                for _, row in overall_df.iterrows():
                    avg = row[metric]
                    error_y.append((avg - row[f"{metric}_ci_lower"], row[f"{metric}_ci_upper"] - avg))
