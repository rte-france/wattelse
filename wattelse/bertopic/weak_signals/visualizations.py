...
def plot_num_topics_and_outliers(topic_models: Dict[pd.Timestamp, BERTopic]) -> None:
    ...
    fig_num_topics.update_layout(title="Number of Topics Detected", xaxis_title="Time Period",
                                 yaxis_title="Number of Topics")
    st.plotly_chart(fig_num_topics, config=PLOTLY_BUTTON_SAVE_CONFIG, use_container_width=True)

    outlier_sizes = [model.get_topic_info().loc[model.get_topic_info()['Topic'] == -1]['Count'].values[0] if -1 in
                                                                                                             model.get_topic_info()[
                                                                                                                 'Topic'].values else 0
                     for model in topic_models.values()]
    fig_outlier_sizes = go.Figure(data=[go.Bar(x=list(topic_models.keys()), y=outlier_sizes)])
    fig_outlier_sizes.update_layout(title="Size of Outlier Topic", xaxis_title="Time Period",
...
def plot_topics_per_timestamp(topic_models: Dict[pd.Timestamp, BERTopic]) -> None:
    ...
    with st.expander("Explore topic models"):
        ...
        st.dataframe(selected_model.doc_info_df[['Paragraph', 'document_id', 'Topic', 'Representation', 'source']],
...
def plot_topic_size_evolution(fig, window_size: int, granularity: int, current_date, min_datetime, max_datetime) -> Tuple[float, float]:
    ...
    columns = ['Topic', 'Sources', 'Source_Diversity', 'Representation', 'Latest_Popularity', 'Docs_Count',
               'Paragraphs_Count', 'Latest_Timestamp', 'Documents']

    noise_topics_df, weak_signal_topics_df, strong_signal_topics_df = classify_signals(topic_sizes, window_start,
                                                                                       window_end, q1, q3)

    st.subheader(":grey[Noise]")
    if not noise_topics_df.empty:
        st.dataframe(noise_topics_df.astype(str)[columns].sort_values(by=['Topic', 'Latest_Popularity'],
    ...
    if not strong_signal_topics_df.empty:
        st.dataframe(strong_signal_topics_df.astype(str)[columns].sort_values(by=['Topic', 'Latest_Popularity'],
                                                                              ascending=[False, False]))
    else:
...
def plot_newly_emerged_topics(all_new_topics_df: pd.DataFrame) -> None:
    ...
    with st.expander("Newly Emerged Topics", expanded=False):
        st.dataframe(all_new_topics_df[
                         ['Topic', 'Count', 'Document_Count', 'Representation', 'Documents', 'Timestamp']].sort_values(
            by=['Timestamp', 'Document_Count'], ascending=[True, False]))
...
def transform_merge_histories_for_sankey(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for timestamp, group in transformed_df.groupby('Timestamp'):
        for _, row in group.iterrows():
            ...
            if (topic1, timestamp_index) in topic1_dest_map:
                ...
                topic1_future_timestamp_indices = [idx for idx in topic1_timestamp_indices[topic1] if
                                                   idx > timestamp_index]
    ...
    transformed_df_new = transformed_df_new.loc[
        transformed_df_new.groupby(['Timestamp', 'Source', 'Destination'])['Count'].idxmin()]
...
def create_sankey_diagram_plotly(all_merge_histories_df: pd.DataFrame, search_term: str = None, max_pairs: int = None):
    ...
    if search_term:
        ...
        def find_connected_nodes(node, connected_nodes):
            if node not in connected_nodes:
                ...
                connected_df = all_merge_histories_df[
                    (all_merge_histories_df['Source'] == node) | (all_merge_histories_df['Destination'] == node)]
        ...
        matching_nodes = set(all_merge_histories_df[all_merge_histories_df['Representation'].apply(
            lambda x: search_term.lower() in str(x).lower())]['Source'])
        ...
        all_merge_histories_df = all_merge_histories_df[(all_merge_histories_df['Source'].isin(connected_nodes)) | (
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            source=[nodes.index(next((node for node in nodes if node['name'] == link['source']), None)) for link in
                    links],
            target=[nodes.index(next((node for node in nodes if node['name'] == link['target']), None)) for link in
                    links],
            value=[link['value'] for link in links],
            customdata=[(link['timestamp'], link['source_topic_id'], link['target_topic_id'], link['representation'],
