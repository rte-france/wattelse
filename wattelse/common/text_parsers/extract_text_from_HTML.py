...
def html_table_to_md(html_table: Tag) -> str:
    ...
    headers = [
        ...
        for th in html_table.find("tr").find_all(["th", "td", "col"])
    ]
    ...
    for row in html_table.find_all("tr")[1:]:
        ...
        if len(data) != len(
            headers
        ):  # TODO: how to handle tables with different row sizes ?
            continue
        else:
...
def html_list_to_md(html_list: Tag) -> str:
    ...
    for line in html_list.find_all(
        "li", recursive=False
    ):  # TODO: handle multi-level lists
