...
if [ -z "$1" ]; then
  # no args provided
  CUDA_VISIBLE_DEVICES=2,1 python3 -m fastchat.serve.model_worker --host $HOST --port $PORT_WORKER --worker-address http://$HOST:$PORT_WORKER --controller-address http://$HOST:$PORT_CONTROLLER --model-path $MODEL --num-gpus 2&
else
  ...
  CUDA_VISIBLE_DEVICES=2,1 python3 -m fastchat.serve.model_worker --host $HOST --port $PORT_WORKER --worker-address http://$HOST:$PORT_WORKER --controller-address http://$HOST:$PORT_CONTROLLER --model-path $MODEL --num-gpus 1 --load-8bit&
fi
