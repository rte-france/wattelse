...
def process_new_data():
    ...
    tm = TopicMetrics(st.session_state["topic_model"], new_topics_over_time)
    ...
    for i, ts_avg in sorted(ts_avg.items()):
        ...
        with st.spinner(f"Simulating batch #{batch_id} as new incoming data..."):
            ...
            topics_over_time_per_batch = [
                {"Topic": t, "Frequency": f, "Words": "", "Timestamp": ts_avg, "Batch": batch_id}
            ...
            topic_metrics = TopicMetrics(st.session_state["topic_model"], new_topics_over_time)
