...
class EmbeddingLoadTest(HttpUser):
    ...
    def encode_task(self):
        ...
        texts = [generate_text(random.randint(1, 10)) for i in range(0,random.randint(1, 10))]
        logger.debug(f"Sending {myuuid}")
        response = self.client.post(ENDPOINT, json={'text': texts, 'show_progress_bar': show_progress_bar})
