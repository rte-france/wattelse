...
series_paragraphs_save_path = (
    "./data/processed_data/series_tokenized_paragraphs_"
    + tokenizer_name.replace("/", "-")
    + ".pickle"
)
...
df_paragraphs["tokenized_paragraphs"] = df_paragraphs.apply(
    lambda row: tokenizer(row.paragraphs), axis=1
)
...
df_paragraphs["n_tokens"] = df_paragraphs.apply(
    lambda row: len(row.tokenized_paragraphs["input_ids"]), axis=1
)
...
hist_plot.savefig(
    "./figures/hist_n_tokens_paragraphs_" + tokenizer_name.replace("/", "-"),
    bbox_inches="tight",
)
